...
//{ LOGICIEL : SSP
//{ © DBsolutions
//{          Procédure : F_Regional_Import
//{          Lundi 31 janvier 2011 à 11:30
//{          
//{          Développement : GABBAY Jean-Guy
//{==================================================}

C_BOOLEAN:C305($0)
C_BOOLEAN:C305($vb_OK; $vb_OKCréer)
C_LONGINT:C283($ii; $ii_Plus; $ii_Cas)
...
Repeat 
	...
	va_Centre_Rep:=<>tt_Conso_Valeur{$vl_OffSet+26}
	If (vl_RéfU_LC>0)
		READ WRITE:C146([LesCentres:9])
		If ((vl_RéfU_LC>=0) & (vl_RéfU_LC<=999999))
			QUERY:C277([LesCentres:9]; [LesCentres:9]LC_RéférenceID:1>0; *)
			QUERY:C277([LesCentres:9];  & ; [LesCentres:9]LC_RéférenceID:1<=999999; *)
			QUERY:C277([LesCentres:9];  & ; [LesCentres:9]LC_Nom:4=<>tt_Conso_Valeur{$vl_OffSet+15})
			MultiSoc_Filter(->[LesCentres:9])
			If (Records in selection:C76([LesCentres:9])=0)
				vl_RéfU_LC:=Uut_Numerote(->[LesCentres:9])
			End if 
		Else 
			QUERY:C277([LesCentres:9]; [LesCentres:9]LC_RéférenceID:1=vl_RéfU_LC)
			MultiSoc_Filter(->[LesCentres:9])
		End if 
		If (Records in selection:C76([LesCentres:9])=0)
			CREATE RECORD:C68([LesCentres:9])
			MultiSoc_Init_Structure(->[LesCentres:9])
			[LesCentres:9]LC_RéférenceID:1:=vl_RéfU_LC
	...
	If ($vb_OKCréer)
		...
		Else 
			[HeberGement:5]HG_FicheNuméro:3:=<>tt_Conso_Valeur{$vl_OffSet+3}
		End if 
	...
	If ($vb_OK)
		...
		[HeberGement:5]HG_Regional_Departement:154:=<>va_RG_Dept
