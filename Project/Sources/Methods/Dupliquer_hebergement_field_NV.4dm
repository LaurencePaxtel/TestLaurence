...
// Date et heure : 10/03/23, 14:42:03
// ----------------------------------------------------
// Méthode : Dupliquer_hebergement_field_NV
// Description
// 
//
// Paramètres
...
C_LONGINT:C283($L_PK; $i; $L_Table; $L_Max_field; $L_Size; $L_column; $L_row)
...
$L_PK:=168  // Numéro de champ de [HeberGement]ID - Get_Primary_Key(Table(->[HeberGement]))

Case of 
	: ($T_Selecteur="load")  // On relit l'objet ou les champs de la table, on place le tout dans une listbox (1 paramètre)
		Dupliquer_hebergement_field_NV("get"; ->$rO_Duplique)
		...
		If ($save_b=True:C214)
			Dupliquer_hebergement_field_NV("save")
		End if 
	...
	: ($T_Selecteur="save")  // On place les colonnes de la listbox dans un objet (1 paramètre)
		...
		Dupliquer_hebergement_field_NV("set"; ->$rO_Duplique)
	: ($T_Selecteur="set") | ($T_Selecteur="get")  // SET : Le contenu de l'objet récupéré de la listbox est stocké dans un enregistrement || GET : Le tableau d'objet dans son enregistrement est placé dans une variable tableau objet
		...
		If ([DePart2:76]Reference_ID:4#-2900)
			Dupliquer_hebergement_field_NV("Charger_record")
		End if 
		...
		If ($T_Selecteur="set")
			...
			OB SET ARRAY:C1227([DePart2:76]Proprietes_Nv:6; "duplique"; $rO_Duplique)
	...
	: ($T_Selecteur="reset")  // vider le champ objet
		...
		OB REMOVE:C1226([DePart2:76]Proprietes_Nv:6; "duplique")
		SAVE RECORD:C53([DePart2:76])
		
		UNLOAD RECORD:C212([DePart2:76])
		
		READ ONLY:C145([DePart2:76])
		Dupliquer_hebergement_field_NV("load")
