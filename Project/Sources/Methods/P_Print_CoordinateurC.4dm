...
//{ © DBsolutions/Paxtel
//{
//{          Procédure : P_Print_CoordinateurS
//{
...
If (OK=1)
	...
	If (Records in selection:C76([DePart:1])>0)
		...
		//FORMULAIRE SORTIE([DéPart];"DP_Stat_Rapport")
		//IMPRIMER SELECTION([DéPart];*)
		If (w1=0)
			...
			//  FORMULAIRE SORTIE([DéPart];"DP_Stat_ParOrientation")
			// IMPRIMER SELECTION([DéPart];*)
			
			
			FORM SET OUTPUT:C54([DePart:1]; "DP_Stat_Groupe")
			PRINT SELECTION:C60([DePart:1]; *)
			
			FORM SET OUTPUT:C54([DePart:1]; "DP_Stat_Famille")
			PRINT SELECTION:C60([DePart:1]; *)
		End if 
		...
		If (w1=0)
			...
			//FORMULAIRE SORTIE([LesCentres];"LC_Prt_StatC1")
			//IMPRIMER SELECTION([LesCentres];*)
			
			
			_O_PAGE SETUP:C299([LesCentres:9]; "LC_Prt_StatC1T")
			PRINT SETTINGS:C106
			If (OK=1)
				Case of 
					: (((rJourNuit1=0) & (rJourNuit2=0)) | ((rJourNuit1=1) & (rJourNuit2=1)))
						FORM SET OUTPUT:C54([LesCentres:9]; "LC_Prt_StatC1T")
					: ((rJourNuit1=1) & (rJourNuit2=0))
						FORM SET OUTPUT:C54([LesCentres:9]; "LC_Prt_StatC1J")
					: ((rJourNuit1=0) & (rJourNuit2=1))
						FORM SET OUTPUT:C54([LesCentres:9]; "LC_Prt_StatC1N")
				End case 
		...
		//  FORMULAIRE SORTIE([DéPart];"DP_Stat_GIP1")
		//IMPRIMER SELECTION([DéPart];*)
		//FORMULAIRE SORTIE([DéPart];"DP_Stat_GIP2")
		// IMPRIMER SELECTION([DéPart];*)
		// FORMULAIRE SORTIE([DéPart];"DP_Stat_GIP3")
		//IMPRIMER SELECTION([DéPart];*)
		//FORMULAIRE SORTIE([DéPart];"DP_Stat_Prestations")
		//IMPRIMER SELECTION([DéPart];*)
		//FORMULAIRE SORTIE([DéPart];"DP_Stat_Appels")
		//IMPRIMER SELECTION([DéPart];*)
	End if 
End if 