...
C_TEXT:C284($path; $query; $main_path; $sub_path; $attribut_cible; $valeur_cible)
C_LONGINT:C283($i; $p; $pci; $pc)
C_BOOLEAN:C305($continuer)
C_OBJECT:C1216($objet; $parent_objet; $value)
...
If ($pci>0)
	...
	If ($pe>0)
		...
		For ($i; 1; Size of array:C274($_item); 1)
			OB_Fixer_objet($1; $main_path+"["+String:C10($_item{$i}-1)+"]."+$sub_path; $value)
		End for 
	...
	While (Position:C15("."; $path)>0)
		...
		Case of 
			: ($pc<$p) & ($pc>0)
				...
				$pcf:=Position:C15("]"; $path; $pc)
				$item:=Substring:C12($path; $pc+1; $pcf-$pc-1)
	...
	If ($continuer)
		...
		Else 
			...
			Case of 
				...
				: ($item_num>0)
					ARRAY OBJECT:C1221($_val; $item_num)
