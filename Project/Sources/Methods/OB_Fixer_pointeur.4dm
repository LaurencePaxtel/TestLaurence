...
If ($pci>0)
	...
	If ($pe>0)
		...
		For ($i; 1; Size of array:C274($_item); 1)
			OB_Fixer_pointeur($1; $main_path+"["+String:C10($_item{$i}-1)+"]."+$sub_path; $value)
		End for 
	End if 
Else 
	...
	If ($continuer)
		...
		Else 
			...
			Case of 
				...
				: ($item_num>0)
					ARRAY POINTER:C280($_val; $item_num)
					$_val{$item_num}:=$value
					OB SET ARRAY:C1227($objet; $path; $_val)
			End case 
		End if 
