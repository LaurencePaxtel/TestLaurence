...
//{ © DBsolutions
//{
//{          Procédure : F_Regional_TransfertData
//{
//{          Mardi 01 février 2011 à 18:28
//{          Modifiée :
...
$vt_Block:=$vt_Block+"Infos : "+<>va_Tab+<>va_T_Site+", Le "+String:C10($2; Interne date court:K1:7)+" à "+String:C10($3; h mn s:K7:1)+<>va_Tab+String:C10($ve_ExportType)+<>va_Tab+<>va_T_Departement+<>va_CR
...
Else 
	$vt_Block:=$vt_Block+"Fiche n°"+<>va_Tab+[HeberGement:5]HG_FicheNuméro:3+Substring:C12(<>va_T_Lettre; 1; 1)+<>va_Tab+String:C10(Table:C252(->[HeberGement:5]))+<>va_Tab+String:C10(Field:C253(->[HeberGement:5]HG_FicheNuméro:3))+<>va_CR
End if 
		...
		If (<>tb_RdsTrans{$vl_TransSTR}{$ii})
			...
			Case of 
				: (ve_Type=Est un champ alpha:K8:1)
					...
					If (Field:C253(<>te_RdsFic{$vl_TransSTR}{$ii}; <>te_RdsRub{$vl_TransSTR}{$ii})=(->[HeberGement:5]HG_FicheNuméro:3))
						If (<>vb_T_ModeCHRS)
						Else 
							$va_Temp:=$va_Temp+Substring:C12(<>va_T_Lettre; 1; 1)
						End if 
					End if 
