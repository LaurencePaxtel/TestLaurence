...
// Date and time: 18-12-18, 08:50:45
// ----------------------------------------------------
// Method: Web_Autocomplete_Heberges
// Description
// 
//
// Parameters
...
ARRAY TEXT:C222($tb_chef_famille; 0)
...
$id_origin:=Num:C11(OB Get:C1224($O_Data; "id"; Est un texte:K8:3))
...
If (Not:C34(Is nil pointer:C315($option_field))) & (Not:C34(Is nil pointer:C315($option_value)))
	QUERY SELECTION:C341($table->; $option_field->=$option_value->)
End if 

ORDER BY:C49($table->; $field_value->; >)

REDUCE SELECTION:C351($table->; 20)
...
Else 
	SELECTION TO ARRAY:C260($field_id->; $tb_id; $field_value->; $tb_value; $field_value_second->; $tb_value_second; [HeBerge:4]HB_DateNéLe:5; $tb_value_date_naissance; [HeBerge:4]HB_CléFam:47; $tb_chef_famille)
	
	For ($i; 1; $size)
		OB SET:C1220($tb_result{$i}; "id"; $tb_id{$i}; "value"; $tb_value{$i}+" "+$tb_value_second{$i}+" "+String:C10($tb_value_date_naissance{$i}); "value_plus"; $tb_chef_famille{$i})
	End for 
