...
Case of 
	: ($1=0)
		ARRAY INTEGER:C220(te_TriSel; 4)
		ARRAY TEXT:C222(ta_TriRub; 4)
		ARRAY POINTER:C280(ta_TriRubPtr; 4)
		ARRAY INTEGER:C220(te_TriSens; 4)
		ta_TriRub{1}:="Numéro de fiche"
		ta_TriRubPtr{1}:=->[Maraude:24]MR_FicheNuméro:3
		te_TriSens{1}:=1
		te_TriSel{1}:=1
		ta_TriRub{2}:="Date"
		ta_TriRubPtr{2}:=->[Maraude:24]MR_Date:4
		te_TriSens{2}:=1
		te_TriSel{2}:=1
		ta_TriRub{3}:="Nom"
		ta_TriRubPtr{3}:=->[Maraude:24]MR_Nom:17
		te_TriSens{3}:=1
		te_TriSel{3}:=1
		ta_TriRub{4}:="C.P."
		ta_TriRubPtr{4}:=->[Maraude:24]MR_ArrondCP:15
		te_TriSens{4}:=1
		te_TriSel{4}:=1
	...
	: ($1=2)
		...
		If ($ik>0)
			...
			For ($ii; 1; Size of array:C274($te_TriSel))
				If ($te_TriSel{$ii}>0)
					If ($te_TriSens{$ii}=1)
						ORDER BY:C49([Maraude:24]; $ta_TriRubPtr{$ii}->; >; *)
					Else 
						ORDER BY:C49([Maraude:24]; $ta_TriRubPtr{$ii}->; <; *)
					End if 
				End if 
			End for 
			ORDER BY:C49([Maraude:24])
		End if 
End case 
