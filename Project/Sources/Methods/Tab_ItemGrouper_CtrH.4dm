...
//{ © DBsolutions/Paxtel
//{
//{          Procédure : Tab_ItemGrouper_CtrH
//{
...
Case of 
	...
	: ($1=2)  //Chargement des données
		...
		vl_NbCentresGpe:=Records in selection:C76([ItemGroupe:43])
		ORDER BY:C49([ItemGroupe:43]; [ItemGroupe:43]IT_Libellé:3; >)
		SELECTION TO ARRAY:C260([ItemGroupe:43]IT_RéférenceID:1; tl_yCtreH_Réf; [ItemGroupe:43]IT_Catégorie:2; ta_yCtreH_Réf; [ItemGroupe:43]IT_Libellé:3; ta_yCtreH; [ItemGroupe:43]IT_Ordre:6; te_yCtreH_Ordre; [ItemGroupe:43]IT_Diviseur:8; te_yCtreH_Div)
		...
		QUERY:C277([ItemGroupe:43];  & ; [ItemGroupe:43]IT_LibelléOrigneID:5>0)
		MultiSoc_Filter(->[ItemGroupe:43])
		vl_NbCentresNA:=Records in selection:C76([ItemGroupe:43])
		ORDER BY:C49([ItemGroupe:43]; [ItemGroupe:43]IT_Libellé:3; >)
		SELECTION TO ARRAY:C260([ItemGroupe:43]IT_RéférenceID:1; tl_FileCtreH_Réf; [ItemGroupe:43]IT_Catégorie:2; ta_FileCtreH_Réf; [ItemGroupe:43]IT_Libellé:3; ta_FileCtreH; [ItemGroupe:43]IT_LibelléOrigine:4; ta_FilexCtreH; [ItemGroupe:43]IT_LibelléOrigneID:5; tl_FilexCtreH_Réf; [ItemGroupe:43]IT_Ordre:6; te_FileCtreH_Ordre; [ItemGroupe:43]IT_Etat:7; te_FileCtreH_Etat; [ItemGroupe:43]IT_Diviseur:8; te_FileCtreH_Div)
		
		Tab_ItemGrouper_CtrH(3; 0; 1)  //Relié ou pas
		Tab_ItemGrouper_CtrH(3; 0; 2)  //Nombre de centres reliés
		...
		Tab_ItemGrouper_CtrH(1; $ii; 1)  //Synchroniser       
		Tab_ItemGrouper_CtrH(4; $ii; 1)  //Clic dans le tableau de gauche
	...
	: ($1=6)  //Tableaux : Affecter
		...
		Case of 
			: ($3=1)
			: ($3=2)
				tl_yCtreH_Réf{$ii}:=[ItemGroupe:43]IT_RéférenceID:1
				ta_yCtreH_Réf{$ii}:=[ItemGroupe:43]IT_Catégorie:2
				ta_yCtreH{$ii}:=[ItemGroupe:43]IT_Libellé:3
				te_yCtreH_Ordre{$ii}:=[ItemGroupe:43]IT_Ordre:6
				te_yCtreH_Etat{$ii}:=[ItemGroupe:43]IT_Etat:7
				te_yCtreH_Div{$ii}:=[ItemGroupe:43]IT_Diviseur:8
				te_yCtreH_Check{$ii}:=0
			: ($3=3)
				tl_FileCtreH_Réf{$ii}:=[ItemGroupe:43]IT_RéférenceID:1
				ta_FileCtreH_Réf{$ii}:=[ItemGroupe:43]IT_Catégorie:2
				ta_FileCtreH{$ii}:=[ItemGroupe:43]IT_Libellé:3
				ta_FilexCtreH{$ii}:=[ItemGroupe:43]IT_LibelléOrigine:4
				tl_FilexCtreH_Réf{$ii}:=[ItemGroupe:43]IT_LibelléOrigneID:5
				te_FileCtreH_Ordre{$ii}:=[ItemGroupe:43]IT_Ordre:6
				te_FileCtreH_Etat{$ii}:=[ItemGroupe:43]IT_Etat:7
				te_FileCtreH_Div{$ii}:=[ItemGroupe:43]IT_Diviseur:8
		End case 
	...
	: ($1=7)  //Tableaux : Affecter plusieurs lignes  :synchro apres modif
		...
		For ($ii; 1; Size of array:C274(tl_FileCtreH_Réf))
			If (ta_FileCtreH{$ii}=va_LibEnum)
				...
				QUERY:C277([ItemGroupe:43]; [ItemGroupe:43]IT_RéférenceID:1=tl_FileCtreH_Réf{$ii})
				MultiSoc_Filter(->[ItemGroupe:43])
				If (Records in selection:C76([ItemGroupe:43])=1)
					Repeat 
						LOAD RECORD:C52([ItemGroupe:43])
					Until (Not:C34(Locked:C147([ItemGroupe:43])))
					[ItemGroupe:43]IT_Libellé:3:=ta_FileCtreH{$ii}
					[ItemGroupe:43]IT_Ordre:6:=te_FileCtreH_Ordre{$ii}
					[ItemGroupe:43]IT_Diviseur:8:=te_FileCtreH_Div{$ii}
		...
		If ($vb_OK)
			...
			If ($ii>0)
				If (i_Confirmer(ta_xCtreH{$2}+" est déjà rattaché à : "+ta_FileCtreH{$ii}+<>va_CR+"Veuillez confirmer le transfert ?"))
					...
					Tab_ItemGrouper_CtrH(22; $2; $3)  //Modifier la liaison
				End if 
			Else   //Pas de lien
				Tab_ItemGrouper_CtrH(21; $2; $3)
			End if 
		End if 
	...
	: ($1=21)  //Effectuer la liaison
		...
		[ItemGroupe:43]IT_Libellé:3:=ta_yCtreH{$3}
		[ItemGroupe:43]IT_LibelléOrigine:4:=ta_xCtreH{$2}
		[ItemGroupe:43]IT_LibelléOrigneID:5:=tl_xCtreH_ID{$2}
		[ItemGroupe:43]IT_Ordre:6:=te_yCtreH_Ordre{$3}
		[ItemGroupe:43]IT_Etat:7:=0
		[ItemGroupe:43]IT_Diviseur:8:=te_yCtreH_Div{$3}
		SAVE RECORD:C53([ItemGroupe:43])
		Tab_ItemGrouper_CtrH(5; 0; 3)
		$ii:=Size of array:C274(tl_FileCtreH_Réf)
		Tab_ItemGrouper_CtrH(6; $ii; 3)
		UNLOAD RECORD:C212([ItemGroupe:43])
		READ ONLY:C145([ItemGroupe:43])
		//Tab_ItemGrouper_CtrH (3;0;1)
		te_xCtreH{$2}:=1  //Signe Plus de la liaison activée 		
		te_xCtreH_Etat{$2}:=1  //Signe X de l'existance de la laison 
		Tab_ItemGrouper_CtrH(3; 0; 2)
		Tab_ItemGrouper_CtrH(4; $2; 1)
	...
	: ($1=22)  //Modifier la liaison
		...
		If (Records in selection:C76([ItemGroupe:43])=1)
			...
			Until (Not:C34(Locked:C147([ItemGroupe:43])))
			[ItemGroupe:43]IT_Libellé:3:=ta_yCtreH{$3}
			//[ItemGroupe]IT_LibelléOrigine:=ta_xCtreH{$2}
			//[ItemGroupe]IT_LibelléOrigneID:=tl_xCtreH_ID{$2}
			[ItemGroupe:43]IT_Ordre:6:=te_yCtreH_Ordre{$3}
			[ItemGroupe:43]IT_Etat:7:=0
			[ItemGroupe:43]IT_Diviseur:8:=te_yCtreH_Div{$3}
			SAVE RECORD:C53([ItemGroupe:43])
	...
	: ($1=40)  //Ajouter une fiche dans la table
		...
		If (OK=1)
			...
			//Tab_ItemGrouper_CtrH (6;Taille tableau(tl_FileCtreH_Réf);3)
			
			//Nouvelle ligne dans le tableau de droite
			Tab_ItemGrouper_CtrH(5; 0; 2)
			//Affecter le tableau de droite
			Tab_ItemGrouper_CtrH(6; Size of array:C274(tl_yCtreH_Réf); 2)
			
			Tab_ItemGrouper_CtrH(1; ta_xCtreH; 1)  //Synchroniser     
			Tab_ItemGrouper_CtrH(4; ta_xCtreH; 1)  //Clic dans le tableau de gauche  
			Tab_ItemGrouper_CtrH(1; Size of array:C274(tl_yCtreH_Réf); 2)  //Synchroniser     
			Tab_ItemGrouper_CtrH(4; Size of array:C274(tl_yCtreH_Réf); 2)  //Clic dans le tableau de droite  
	...
	: ($1=50)  //Modifier une fiche dans la table
		...
		If ($vb_OK)
			READ WRITE:C146([ItemGroupe:43])
			QUERY:C277([ItemGroupe:43]; [ItemGroupe:43]IT_RéférenceID:1=tl_yCtreH_Réf{ta_yCtreH})
			MultiSoc_Filter(->[ItemGroupe:43])
			If (Records in selection:C76([ItemGroupe:43])=1)
				va_LibEnum:=""
				FORM SET INPUT:C55([ItemGroupe:43]; "IT_EcranSaisie")
				...
				If (OK=1)
					...
					Tab_ItemGrouper_CtrH(6; ta_yCtreH; 2)
					//Modifications générales du libellé de l'item de regroupement
					Tab_ItemGrouper_CtrH(7; ta_yCtreH; 2)
				End if 
				UNLOAD RECORD:C212([ItemGroupe:43])
				CLOSE WINDOW:C154($vl_Fenetre)
			End if 
	...
	: ($1=60)  //Supprimer une fiche dans la table
		...
		If ($vb_OK)
			If (i_Confirmer("Confirmez-vous la suppression de l'item : "+ta_yCtreH{$3}))
				...
				Tab_ItemGrouper_CtrH(8; $3; 2)
				//Synchroniser l'ensemble
				vl_NbCentresGpe:=Size of array:C274(tl_yCtreH_Réf)
				If ($3>vl_NbCentresGpe)
					$3:=vl_NbCentresGpe
				End if 
				Tab_ItemGrouper_CtrH(1; $3; 2)
				Tab_ItemGrouper_CtrH(4; $3; 2)
				Tab_ItemGrouper_CtrH(3; $2; 1)
				Tab_ItemGrouper_CtrH(3; $3; 2)
			End if 
		End if 
		...
		Case of 
			...
			: (Size of array:C274(ta_yCtreH)>0)
				...
				For ($ii; 1; Size of array:C274(ta_yCtreH))
					QUERY:C277([ItemGroupe:43]; [ItemGroupe:43]IT_Catégorie:2=<>ta_ItemGroupe{2}; *)
					QUERY:C277([ItemGroupe:43];  & ; [ItemGroupe:43]IT_Libellé:3=ta_yCtreH{$ii})
					MultiSoc_Filter(->[ItemGroupe:43])
					If (Records in selection:C76([ItemGroupe:43])>0)
