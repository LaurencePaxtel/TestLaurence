...
If (Size of array:C274(ta_Centre)>0)
	If (ta_Centre>0)
		va_TitreCtr:=ta_Centre{ta_Centre}
	End if 
End if 
	...
	If (Records in selection:C76([DePart:1])>0)
		...
		If (sNbOrHB=1)  //Nombre d'orientations par personne
			...
			If (Size of array:C274(td_ORnb)<=vl_NbLigneP)
				FORM SET OUTPUT:C54([DePart:1]; "DP_Stat_ParDemandeur1")
			...
			Else 
				...
				End if 
				For ($ii; 1; $vlLeNbréél)
					FORM SET OUTPUT:C54([DePart:1]; "DP_Stat_ParDemandeur2")
					PRINT SELECTION:C60([DePart:1]; *)
					If ($ii<$vlLeNbréél)
				...
				If (Records in selection:C76([HeberGement:5])>0)
					...
					If (Records in set:C195("E_HBTemp2")>0)
						...
						If (sNbParHB=1)
							...
							QR REPORT:C197([HeberGement:5]; "StatCTR")
						...
						If (sHGliste=1)
							...
							If (Records in selection:C76([HeBerge:4])>0)
								...
								If (wNom1=1)
									FORM SET OUTPUT:C54([HeBerge:4]; "HB_Prt_StatAvecNom")  //[HéBergé];"FL_StatCtr")
								Else 
									FORM SET OUTPUT:C54([HeBerge:4]; "HB_Prt_StatSansNom")  //[HéBergé];"FL_StatCtr2")
								End if 
