...
$pc:=Position:C15("["; $path)
If ($pc=0)
	If ($last_type=Est un tableau objet:K8:28)
		If ($path="length")
			$resultat:=Size of array:C274($_objet)
		Else 
			$resultat:=0
		End if 
	Else 
		$resultat:=OB Get:C1224($objet; $path; Est un entier long:K8:6)
	End if 
Else 
	$attribut:=Substring:C12($path; 1; $pc-1)
