...
If (OK=1)
	...
	If (OK=1)
		...
		If ($vl_TailTab>0)
			$vt_Texte:="Hébergements"+<>va_TAB+"Distribués"+<>va_TAB+"%Distribués"+<>va_TAB+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
			...
			C_LONGINT:C283($ii)
			For ($ii; 1; $vl_TailTab)
				$vt_Texte:=<>ta_TBEtCiv{$ii}+<>va_TAB+String:C10(t_ET_Dis{$ii})+<>va_TAB+String:C10(t_ET_DisPc{$ii})+<>va_CR
				If (<>vb_CestUnMac)
					SEND PACKET:C103($h_RefDoc; $vt_Texte)
				Else 
					SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
				End if 
			End for 
			$vt_Texte:="TOTAUX"+<>va_TAB+String:C10(vl_ET_Dis)+<>va_TAB+String:C10(vr_ET_DisPc)+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
