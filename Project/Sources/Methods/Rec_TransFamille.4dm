//%attributes = {}
//{==================================================}
//{ LOGICIEL : Samu Social de Paris	
//{ © DBsolutions/Paxtel
//{
//{          Procédure : Rec_TransFamille
//{
//{          Vendredi 23 février 2007 à 18:12:00
//{          Modifiée :
//{          Développement : GABBAY Jean-Guy
//{==================================================}

C_LONGINT:C283($1)
C_POINTER:C301($2)
C_POINTER:C301($3)
C_POINTER:C301($4)
C_LONGINT:C283($5)

Case of 
	: ($1=0)
		ARRAY TEXT:C222(ta_ValFam; 1)
		ta_ValFam{1}:="Toutes"
		ta_ValFam:=1
		ta_ValFam{0}:=ta_ValFam{1}
		va_Titre:="Recherche des transferts : "+("Matin"*Num:C11(Vb_JouNuit=False:C215))+("Soir"*Num:C11(Vb_JouNuit=True:C214))
		i_Message(va_Titre)
		
		CREATE EMPTY SET:C140([HeberGement:5]; "E_Hébergé")
		CREATE EMPTY SET:C140([HeberGement:5]; "E_HébergéInter")
		CREATE EMPTY SET:C140([HeberGement:5]; "E_HébergéTemp")
		CREATE EMPTY SET:C140([LesCentres:9]; "E_Centres")
		
		
		QUERY:C277([HeberGement:5]; [HeberGement:5]HG_Date:4=$2->; *)
		QUERY:C277([HeberGement:5];  & ; [HeberGement:5]HG_Nuit:2=$3->; *)
		QUERY:C277([HeberGement:5];  & ; [HeberGement:5]HG_NuitenCours:92=1; *)
		
		QUERY:C277([HeberGement:5];  & ; [HeberGement:5]HG_LC_ID:61>=<>vl_T_CentreCodeDebut; *)
		
		QUERY:C277([HeberGement:5];  & ; [HeberGement:5]HG_LC_ID:61<=<>vl_T_CentreCodeFin; *)
		
		QUERY:C277([HeberGement:5];  & ; [HeberGement:5]HG_Cloturée:67=False:C215)
		
		
		MultiSoc_Filter(->[HeberGement:5])
		//  Si (◊vb_T_FullDuplex)
		//    CHERCHER([HéberGement]; & ;[HéberGement]HG_Cloturée=Faux)
		//Sinon 
		//CHERCHER([HéberGement]; & ;[HéberGement]HG_Cloturée=Vrai)
		// Fin de si 
		
		CREATE SET:C116([HeberGement:5]; "E_Hébergé")
		vl_NbHB:=Records in selection:C76([HeberGement:5])
		RELATE MANY SELECTION:C340([LesCentres:9]LC_RéférenceID:1)
		
		// vl_NbCT:=Enregistrements trouves([LesCentres])
		//NOMMER ENSEMBLE([LesCentres];"E_Centres")
		
		QUERY SELECTION:C341([LesCentres:9]; [LesCentres:9]LC_Répertoire:54#"115@")
		vl_NbCT:=Records in selection:C76([LesCentres:9])
		CREATE SET:C116([LesCentres:9]; "E_Centres")
		
		
		ORDER BY:C49([LesCentres:9]; [LesCentres:9]LC_Nom:4; >)
		SELECTION TO ARRAY:C260([LesCentres:9]LC_RéférenceID:1; tl_CTRéf; [LesCentres:9]LC_Nom:4; ta_CTNom; [LesCentres:9]LC_Lieu:5; ta_CTLieu; [LesCentres:9]LC_Téléphone:29; ta_CTTél; [LesCentres:9]LC_Télécopie2:31; ta_CTFax; [LesCentres:9]LC_DispoNuit:22; tl_CTCH; [LesCentres:9]LC_Restantes:23; tl_CTCHAt; [LesCentres:9]LC_Répertoire:54; ta_CTRépertoire)
		
		ARRAY INTEGER:C220(tl_CTSél; Size of array:C274(ta_CTNom))
		ta_CTNom:=0
		
		CREATE EMPTY SET:C140([HeberGement:5]; "E_HébergéInter")
		CREATE EMPTY SET:C140([HeberGement:5]; "E_HébergéTemp")
		
		For ($ii; 1; Size of array:C274(ta_CTNom))
			USE SET:C118("E_Hébergé")
			QUERY SELECTION:C341([HeberGement:5]; [HeberGement:5]HG_LC_ID:61=tl_CTRéf{$ii})
			tl_CTCHAt{$ii}:=Records in selection:C76([HeberGement:5])
			If (tl_CTCHAt{$ii}>0)
				CREATE SET:C116([HeberGement:5]; "E_HébergéTemp")
				UNION:C120("E_HébergéInter"; "E_HébergéTemp"; "E_HébergéInter")
				
			End if 
			tl_CTSél{$ii}:=0
		End for 
		INTERSECTION:C121("E_Hébergé"; "E_HébergéInter"; "E_Hébergé")
		USE SET:C118("E_Hébergé")
		CLEAR SET:C117("E_HébergéInter")
		CLEAR SET:C117("E_HébergéTemp")
		
		//   TRIER([HéberGement];[HéberGement]HG_FicheNuméro;>)
		ORDER BY:C49([HeberGement:5]; [HeberGement:5]HG_Nom:21; >; *)
		ORDER BY:C49([HeberGement:5]; [HeberGement:5]HG_Prénom:22; >)
		SORT ARRAY:C229(ta_CTNom; ta_CTLieu; ta_CTTél; ta_CTFax; tl_CTCH; tl_CTCHAt; ta_CTRépertoire; tl_CTRéf; >)
		CLOSE WINDOW:C154
		
		OBJECT SET ENABLED:C1123(b_VoirNotes; False:C215)
		OBJECT SET ENABLED:C1123(b_VoirSP; False:C215)
		OBJECT SET ENABLED:C1123(b_DSOui; False:C215)
		OBJECT SET ENABLED:C1123(b_DSNon; False:C215)
		If (Records in selection:C76([HeberGement:5])>0)
			OBJECT SET ENABLED:C1123(b_DSVoir; True:C214)
			DISTINCT VALUES:C339([HeberGement:5]HG_FamClé:104; ta_ValFam)
			INSERT IN ARRAY:C227(ta_ValFam; Size of array:C274(ta_ValFam)+1)
			ta_ValFam{Size of array:C274(ta_ValFam)}:="Toutes"
			ta_ValFam:=Size of array:C274(ta_ValFam)
			ta_ValFam{0}:=ta_ValFam{ta_ValFam}
		Else 
			OBJECT SET ENABLED:C1123(b_DSVoir; False:C215)
		End if 
		If (Size of array:C274(ta_CTNom)>0)
			OBJECT SET ENABLED:C1123(b_FaxHB; True:C214)
			OBJECT SET ENABLED:C1123(b_PrintHB; True:C214)
		Else 
			OBJECT SET ENABLED:C1123(b_FaxHB; False:C215)
			OBJECT SET ENABLED:C1123(b_PrintHB; False:C215)
		End if 
		
		
	: ($1=1)
		USE SET:C118("E_Hébergé")
		If (ta_ValFam{0}="Toutes")
		Else 
			QUERY SELECTION:C341([HeberGement:5]; [HeberGement:5]HG_FamClé:104=ta_ValFam{0})
		End if 
		
		//   TRIER([HéberGement];[HéberGement]HG_FicheNuméro;>)
		ORDER BY:C49([HeberGement:5]; [HeberGement:5]HG_Nom:21; >; *)
		ORDER BY:C49([HeberGement:5]; [HeberGement:5]HG_Prénom:22; >)
		vl_NbHB:=Records in selection:C76([HeberGement:5])
		OBJECT SET ENABLED:C1123(b_VoirNotes; False:C215)
		OBJECT SET ENABLED:C1123(b_VoirSP; False:C215)
		OBJECT SET ENABLED:C1123(b_DSOui; False:C215)
		OBJECT SET ENABLED:C1123(b_DSNon; False:C215)
		If (Records in selection:C76([HeberGement:5])>0)
			OBJECT SET ENABLED:C1123(b_DSVoir; True:C214)
		Else 
			OBJECT SET ENABLED:C1123(b_DSVoir; False:C215)
		End if 
		If (Size of array:C274(ta_CTNom)>0)
			OBJECT SET ENABLED:C1123(b_FaxHB; True:C214)
			OBJECT SET ENABLED:C1123(b_PrintHB; True:C214)
		Else 
			OBJECT SET ENABLED:C1123(b_FaxHB; False:C215)
			OBJECT SET ENABLED:C1123(b_PrintHB; False:C215)
		End if 
		If (Size of array:C274(ta_CTNom)>0)
			ta_CTNom:=0
		End if 
		
		
	: ($1=2)
		
		
		If ($5>0)
			USE SET:C118("E_Hébergé")
			QUERY SELECTION:C341([HeberGement:5]; [HeberGement:5]HG_LC_ID:61=tl_CTRéf{$5})
			If (ta_ValFam{0}="Toutes")
			Else 
				QUERY SELECTION:C341([HeberGement:5]; [HeberGement:5]HG_FamClé:104=ta_ValFam{0})
			End if 
		Else 
			USE SET:C118("E_Hébergé")
			If (ta_ValFam{0}="Toutes")
			Else 
				QUERY SELECTION:C341([HeberGement:5]; [HeberGement:5]HG_FamClé:104=ta_ValFam{0})
			End if 
		End if 
		//   TRIER([HéberGement];[HéberGement]HG_FicheNuméro;>)
		ORDER BY:C49([HeberGement:5]; [HeberGement:5]HG_Nom:21; >; *)
		ORDER BY:C49([HeberGement:5]; [HeberGement:5]HG_Prénom:22; >)
		vl_NbHB:=Records in selection:C76([HeberGement:5])
		OBJECT SET ENABLED:C1123(b_VoirNotes; False:C215)
		OBJECT SET ENABLED:C1123(b_VoirSP; False:C215)
		OBJECT SET ENABLED:C1123(b_DSOui; False:C215)
		OBJECT SET ENABLED:C1123(b_DSNon; False:C215)
		If (Records in selection:C76([HeberGement:5])>0)
			OBJECT SET ENABLED:C1123(b_DSVoir; True:C214)
		Else 
			OBJECT SET ENABLED:C1123(b_DSVoir; False:C215)
		End if 
		If (Size of array:C274(ta_CTNom)>0)
			OBJECT SET ENABLED:C1123(b_FaxHB; True:C214)
			OBJECT SET ENABLED:C1123(b_PrintHB; True:C214)
		Else 
			OBJECT SET ENABLED:C1123(b_FaxHBS; False:C215)
			OBJECT SET ENABLED:C1123(b_FaxHB; False:C215)
			OBJECT SET ENABLED:C1123(b_PrintHB; False:C215)
		End if 
		
		
		
	: ($1=3)
		USE SET:C118("E_Hébergé")
		If (ta_ValFam{0}="Toutes")
		Else 
			QUERY SELECTION:C341([HeberGement:5]; [HeberGement:5]HG_FamClé:104=ta_ValFam{0})
		End if 
		If (ta_CTNom>0)
			QUERY SELECTION:C341([HeberGement:5]; [HeberGement:5]HG_LC_ID:61=tl_CTRéf{ta_CTNom})
		End if 
		//   TRIER([HéberGement];[HéberGement]HG_FicheNuméro;>)
		ORDER BY:C49([HeberGement:5]; [HeberGement:5]HG_Nom:21; >; *)
		ORDER BY:C49([HeberGement:5]; [HeberGement:5]HG_Prénom:22; >)
		vl_NbHB:=Records in selection:C76([HeberGement:5])
		OBJECT SET ENABLED:C1123(b_VoirNotes; False:C215)
		OBJECT SET ENABLED:C1123(b_VoirSP; False:C215)
		OBJECT SET ENABLED:C1123(b_DSOui; False:C215)
		OBJECT SET ENABLED:C1123(b_DSNon; False:C215)
		If (Records in selection:C76([HeberGement:5])>0)
			OBJECT SET ENABLED:C1123(b_DSVoir; True:C214)
		Else 
			OBJECT SET ENABLED:C1123(b_DSVoir; False:C215)
		End if 
		If (Size of array:C274(ta_CTNom)>0)
			OBJECT SET ENABLED:C1123(b_FaxHB; True:C214)
			OBJECT SET ENABLED:C1123(b_PrintHB; True:C214)
		Else 
			OBJECT SET ENABLED:C1123(b_FaxHB; False:C215)
			OBJECT SET ENABLED:C1123(b_PrintHB; False:C215)
		End if 
		
		
		
End case 