...
//{ © DBsolutions/Paxtel
//{          Procédure : F_SIAO_DansHebergement
//{          Mercredi 28 décembre 2011 à 15:20:00
//{          Modifiée :
...
Case of 
	: ($1="Declarer")
		C_LONGINT:C283(vl_SiaoHG_Ref; vl_SiaoHG_Indice; vl_SiaoHG_Cas; vl_SiaoHG_RefSIAO; vl_SiaoHG_Rang)
	...
	: ($1="Charger")
		If (<>vb_T_DosSIAO)
			...
			If (Records in selection:C76([SIAO_Personnes:51])>0)
				...
				ARRAY TEXT:C222(ta_SiaoHG_Etat; $vl_fiches)
				...
				For ($ii; 1; $vl_fiches)
					...
					If (Records in selection:C76([SIAO:50])=1)
						...
						ta_SiaoHG_Etat{$ii}:=[SIAO:50]Si_Etat_Type:41
					Else 
					...
					End if 
				End for 
				SORT ARRAY:C229(td_SiaoHG_Date; tl_SiaoHG_RefHB; tl_SiaoHG_RefSIAO; ta_SiaoHG_Theme; ta_SiaoHG_No; ta_SiaoHG_Type; ta_SiaoHG_Etat; <)
				<>vt_HG_SiaoNo:=ta_SiaoHG_Theme{1}+Char:C90(13)+ta_SiaoHG_No{1}
			Else 
				$vb_OK:=F_SIAO_DansHebergement("Vider")
			End if 
			...
			Case of 
				...
				: ($vl_fiches=1)
					...
					vt_AccesDossierSIAO_HG:="SIAO n° "+ta_SiaoHG_No{1}+" - "+ta_SiaoHG_Theme{1}+" - "+ta_SiaoHG_Etat{1}
			...
			If (7=8)
				If (i_Confirmer("Créer un n° de dossier SIAO pour le demandeur ?"))
					If ([HeberGement:5]HG_HB_ID:19>0)
						...
						If (F_SIAO_DansHebergement("Creer"))
						End if 
		...
		Else 
			...
			Else 
				If ([HeberGement:5]HG_DossierSIAO:164>"")
					...
					//1 INSERTION/2 URGENCE/3 Codemandeur
					$vb_OK:=(($va_Cas="1") | ($va_Cas="2") | ($va_Cas="3"))
					If ($vb_OK)
						Case of 
							: ($va_Cas="1")  //Urgence
								vl_SiaoHG_Cas:=1
								$vb_OK:=F_SIAO_DansHebergement("Conso1et2"; ->vl_SiaoHG_Cas)
							: ($va_Cas="2")  //Insertion
								vl_SiaoHG_Cas:=2
								$vb_OK:=F_SIAO_DansHebergement("Conso1et2"; ->vl_SiaoHG_Cas)
							: ($va_Cas="3")  //Co-Demandeur
								vl_SiaoHG_Cas:=3
								$vb_OK:=F_SIAO_DansHebergement("Conso3"; ->[HeberGement:5]HG_FamClé:104; ->vl_SiaoHG_Cas)
	...
	: ($1="Conso")
		// F_SIAO_DansHebergement ("Conso";
		//->[HeberGement]HG_HB_ID
		//->[HeberGement]HG_DossierSIAO
		//->[HeberGement]HG_FamClé
	...
	: ($1="Conso1et2")
		...
		[SIAO_Personnes:51]Sp_Genre:10:=F_EtatCivilGenre([HeberGement:5]HG_EtatCivil:20)
	...
	: ($1="Conso3")
		//[HeberGement]HG_FamClé = $2»    
		vl_SiaoHG_RefSIAO:=0
		If ($2->>"")
			QUERY:C277([HeBerge:4]; [HeBerge:4]HB_CléFam:47=$2->)
			MultiSoc_Filter(->[HeBerge:4])
			If (Records in selection:C76([HeBerge:4])=1)
				QUERY:C277([SIAO_Personnes:51]; [SIAO_Personnes:51]Sp_Personne_ID:3=[HeBerge:4]HB_ReferenceID:1)
				MultiSoc_Filter(->[SIAO_Personnes:51])
				If (Records in selection:C76([SIAO_Personnes:51])>0)
					vl_SiaoHG_RefSIAO:=[SIAO_Personnes:51]Sp_SIAO_ID:2
					QUERY:C277([SIAO_Personnes:51]; [SIAO_Personnes:51]Sp_SIAO_ID:2=vl_SiaoHG_RefSIAO)
					ORDER BY:C49([SIAO_Personnes:51]; [SIAO_Personnes:51]Sp_Rang:7; <)
					vl_SiaoHG_Rang:=[SIAO_Personnes:51]Sp_Rang:7
					vl_SiaoHG_Rang:=vl_SiaoHG_Rang+1
				End if 
			End if 
		End if 
		If (vl_SiaoHG_RefSIAO>0)
			READ WRITE:C146([SIAO_Personnes:51])
			CREATE RECORD:C68([SIAO_Personnes:51])
			MultiSoc_Init_Structure(->[SIAO_Personnes:51])
			[SIAO_Personnes:51]Sp_RéférenceID:1:=Uut_Numerote(->[SIAO_Personnes:51])
			
			[SIAO_Personnes:51]Sp_SIAO_ID:2:=vl_SiaoHG_RefSIAO
			[SIAO_Personnes:51]Sp_Personne_ID:3:=[HeberGement:5]HG_HB_ID:19
			[SIAO_Personnes:51]Sp_Famille:4:="SITUATION DE FAMILLE"
			[SIAO_Personnes:51]Sp_Theme:5:="ELEMENTS DE LA DEMANDE"
			
			[SIAO_Personnes:51]Sp_SousTheme:6:="Co-Demandeur"
			
			[SIAO_Personnes:51]Sp_Rang:7:=vl_SiaoHG_Rang
			
			[SIAO_Personnes:51]Sp_EtatCivil:8:=[HeberGement:5]HG_EtatCivil:20
			[SIAO_Personnes:51]Sp_Civilite:9:=""
			[SIAO_Personnes:51]Sp_Genre:10:=F_EtatCivilGenre([HeberGement:5]HG_EtatCivil:20)
			[SIAO_Personnes:51]Sp_Nom:11:="XXXX"
			[SIAO_Personnes:51]Sp_Nom_de_jeune_fille:12:=""
			[SIAO_Personnes:51]Sp_Prenom:13:="xxxx"
			[SIAO_Personnes:51]Sp_Date_de_naissance:14:=[HeberGement:5]HG_DateNéLe:24
			[SIAO_Personnes:51]Sp_Age:15:=0
			[SIAO_Personnes:51]Sp_Age_plus:16:="MiseàJour"
	...
	: ($1="Conso4")  //comme le Conso3 mais en modifiant dans le dossier
		//    
		//[HeberGement]HG_FamClé = $2»    
		vl_SiaoHG_RefSIAO:=0
		If ($2->>"")
		...
		If (vl_SiaoHG_RefSIAO>0)
			...
			READ WRITE:C146([SIAO:50])
			QUERY:C277([SIAO:50]; [SIAO:50]Si_RéférenceID:1=vl_SiaoHG_RefSIAO)
			MultiSoc_Filter(->[SIAO:50])
			If (Records in selection:C76([SIAO:50])=1)
				Repeat 
					IDLE:C311
					LOAD RECORD:C52([SIAO:50])
				Until (Not:C34(Locked:C147([SIAO:50])))
				If ([SIAO:50]Si_Demande_Type:5#2)
					[SIAO:50]Si_Demande_Type:5:=2
					SAVE RECORD:C53([SIAO:50])
				End if 
			End if 
			UNLOAD RECORD:C212([SIAO:50])
