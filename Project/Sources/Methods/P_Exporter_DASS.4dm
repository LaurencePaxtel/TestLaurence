...
CONFIRM:C162("Exporter les statistiques DASS en mode texte sur disque."+<>va_CR+"Confirmez-vous ?")
If (OK=1)
	...
	If (OK=1)
		...
		$vt_Texte:="Statistiques DASS du "+String:C10(vd_Date1; 4)+" au "+String:C10(vd_Date2; 4)+<>va_CR+""+<>va_CR
		$vt_Texte:=$vt_Texte+<>va_TAB+"Disponible"+<>va_TAB+"Distribués"+<>va_TAB+"Restants"+<>va_TAB+"%"+<>va_CR
		$vt_Texte:=$vt_Texte+"Nombre de lits"
		$vt_Texte:=$vt_Texte+<>va_TAB+String:C10(vl_lit_At)+<>va_TAB+String:C10(vl_lit_Dist)+<>va_TAB+String:C10(vl_lit_Rest)+<>va_TAB+String:C10(vr_lit_Pc)+<>va_CR+<>va_CR
		If (<>vb_CestUnMac)
		...
		If ($vl_TailTab>0)
			$vt_Texte:="Signalement"+<>va_TAB+"Nombre"+<>va_TAB+"Pourcentage"+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
		...
		If ($vl_TailTab>0)
			$vt_Texte:="C. Hébergement"+<>va_TAB+"HS"+<>va_TAB+"HC"+<>va_TAB+"HE"+<>va_TAB+"HF"+<>va_TAB+"FS"+<>va_TAB+"FC"+<>va_TAB+"FE"+<>va_TAB+"FF"+<>va_TAB+"EP"+<>va_TAB+"MI"+<>va_TAB+"TOTAL"+<>va_TAB+"Distribué"+<>va_TAB+"Restant"+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
		...
		If ($vl_TailTab>0)
			$vt_Texte:="Orientation1"+<>va_TAB+"TOTAL"+<>va_TAB+"Pourcentage"+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
		...
		If ($vl_TailTab>0)
			$vt_Texte:="Localisation"+<>va_TAB+"TOTAL"+<>va_TAB+"Pourcentage"+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
		...
		If ($vl_TailTab>0)
			$vt_Texte:="Autres solutions"+<>va_TAB+"TOTAL"+<>va_TAB+"Pourcentage"+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
		...
		If ($vl_TailTab>0)
			$vt_Texte:="Véhicule"+<>va_TAB+"TOTAL"+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
		...
		If ($vl_TailTab>0)
			$vt_Texte:="Demande de prestations"+<>va_TAB+"Nombre"+<>va_TAB+"Pourcentage"+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
			End if 
			For ($ii; 1; $vl_TailTab)
				$vt_Texte:=<>ta_DemP{$ii}+<>va_TAB+String:C10(t_DP_Dem{$ii})+<>va_TAB+String:C10(t_DP_DemPc{$ii})+<>va_CR
				If (<>vb_CestUnMac)
					SEND PACKET:C103($h_RefDoc; $vt_Texte)
				Else 
					SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
				End if 
			End for 
			$vt_Texte:="TOTAUX"+<>va_TAB+String:C10(vl_DP_Dem)+<>va_TAB+String:C10(vr_DP_DemPc)+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
		...
		If ($vl_TailTab>0)
			$vt_Texte:="Adresses utiles"+<>va_TAB+"Nombre"+<>va_TAB+"Pourcentage"+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
			End if 
			For ($ii; 1; $vl_TailTab)
				$vt_Texte:=<>ta_AdUtil{$ii}+<>va_TAB+String:C10(t_AU_Dem{$ii})+<>va_TAB+String:C10(t_AU_DemPc{$ii})+<>va_CR
				If (<>vb_CestUnMac)
					SEND PACKET:C103($h_RefDoc; $vt_Texte)
				Else 
					SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
				End if 
			End for 
			$vt_Texte:="TOTAUX"+<>va_TAB+String:C10(vl_AU_Dem)+<>va_TAB+String:C10(vr_AU_DemPc)+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
		...
		If ($vl_TailTab>0)
			$vt_Texte:="Situations particulières"+<>va_TAB+"Nombre"+<>va_TAB+"Pourcentage"+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
			End if 
			For ($ii; 1; $vl_TailTab)
				$vt_Texte:=<>ta_SitPar{$ii}+<>va_TAB+String:C10(t_SP_Dem{$ii})+<>va_TAB+String:C10(t_SP_DemPc{$ii})+<>va_CR
				If (<>vb_CestUnMac)
					SEND PACKET:C103($h_RefDoc; $vt_Texte)
				Else 
					SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
				End if 
			End for 
			$vt_Texte:="TOTAUX"+<>va_TAB+String:C10(vl_SP_Dem)+<>va_TAB+String:C10(vr_SP_DemPc)+<>va_CR
			If (<>vb_CestUnMac)
				SEND PACKET:C103($h_RefDoc; $vt_Texte)
			Else 
				SEND PACKET:C103($h_RefDoc; _O_Mac to Win:C463($vt_Texte))
