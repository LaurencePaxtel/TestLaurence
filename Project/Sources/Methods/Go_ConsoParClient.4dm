...
If (vb_SurServeur)
	...
	//DébutInitConso115 (4)
	DébutInitConsoClient(4)
	
Else 
	<>vb_Conso115Clt:=i_Confirmer("Lancer la consolidation (2 ème confirmation) ?")
End if 
...
If (<>vb_Conso115Clt)
	...
	Repeat 
		...
		//•(2) Une condition
		If (vb_SurServeur)
			If (vd_HeureRéelle>=<>vh_Conso115Clt1) & (vd_HeureRéelle<=<>vh_Conso115Clt2)
				vb_Conso:=True:C214
			Else 
				vb_Conso:=False:C215
			End if 
		...
		vb_OK:=(<>vb_Conso115Clt=False:C215)
		If (<>vb_Conso115Clt)
			<>va_Conso_ChronoTrans:=<>va_Conso_ChronoTrans+<>va_CR
			<>va_Conso_ChronoTrans:=<>va_Conso_ChronoTrans+"Process endormi …"
			POST OUTSIDE CALL:C329(<>PR_ConsoClient)
			
			DELAY PROCESS:C323(Current process:C322; <>vl_Conso115CltTicks)
			//DébutInitConso115 (4)
			DébutInitConsoClient(4)
			vb_OK:=(<>vb_Conso115Clt=False:C215)
		End if 
		If (vb_OK=False:C215)
			<>va_Conso_ChronoTrans:="Process réveillé …"+<>va_CR
			<>va_Conso_ChronoTrans:=<>va_Conso_ChronoTrans+"Consolidation sur le poste : le "+String:C10(Current date:C33; Interne date court:K1:7)+" à "+String:C10(Current time:C178)
			POST OUTSIDE CALL:C329(<>PR_ConsoClient)
		End if 
