...
Case of 
	: ($vl_EventFF=Sur double clic:K2:5)
		...
		Case of 
			: ($P_Col_current=(->te_Centre))
				
				If (Size of array:C274(te_Centre)>0)
					
					If (te_Centre>0)
						
						If (te_Centre{te_Centre}=1)
							te_Centre{te_Centre}:=0
						Else 
							te_Centre{te_Centre}:=1
						End if 
			...
			: ($P_Col_current=(->ta_Centre))
				
				If (Size of array:C274(ta_Centre)>0)
					
					If (ta_Centre>0)
						
						If (te_Centre{ta_Centre}=1)
							te_Centre{ta_Centre}:=0
						Else 
							te_Centre{ta_Centre}:=1
						End if 
