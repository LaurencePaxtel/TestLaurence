	...
	If (tt_TransDocu>0)
		If (te_TransDocuCas{tt_TransDocu}=0)
			...
			If (Records in selection:C76([HeBerge:4])=1)
				...
				QUERY:C277([HeberGement:5];  & [HeberGement:5]HG_Date:4>=<>vd_Trans_FicheDate)
				...
				<>va_Trans_CleFamCHRS:=[HeberGement:5]HG_FamClé:104
			Else 
			...
			Else 
				If (<>vb_Trans_FicheYaDéja)
					<>vt_Trans_Infos:="ATTENTION"+<>va_CR+"Ce demandeur a déjà des hébergements pour les dates supérieures ou égales au "+String:C10(<>vd_Trans_FicheDate; Système date long:K1:3)
				End if 
			End if 
