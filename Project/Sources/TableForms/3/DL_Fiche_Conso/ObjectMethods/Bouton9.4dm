		...
		If ($vb_OK)
			If (i_Confirmer("Document :"+<>ta_ConsoB_Document{<>ta_ConsoB_Document}+<>va_CR+"Signaler comme lu et dépalcer"))
				...
				If (<>va_ConsoB_115)
					...
					vt_DTitreDocumentLu:=<>vt_T_CheminRép+<>va_Rép_TransReportLU+Séparateur dossier:K24:12
					$vb_OK:=F_Report_FullDuplexDossier(12; vt_DTitreDocumentLu)
				Else 
					//• On teste l'existence du dossier  Lu       du dossier sélectionné
					vt_DTitreDocumentLu:=vt_DossierChemin+"LU"+Séparateur dossier:K24:12
					$vb_OK:=F_Report_FullDuplexDossier(12; vt_DTitreDocumentLu)
				End if 
				...
				If ($vb_OK)
					If (<>va_ConsoB_115)
						$va_Temp:=Substring:C12(<>ta_ConsoB_Document{<>ta_ConsoB_Document}; 1; 3)
						If ($va_Temp[[3]]="")
							$va_Temp:=Substring:C12($va_Temp; 1; 2)
						End if 
						
						//• On teste l'existence dans dossier lu du dossier site
						vt_DTitreDocumentLu:=vt_DTitreDocumentLu+$va_Temp+Séparateur dossier:K24:12
						$vb_OK:=F_Report_FullDuplexDossier(12; vt_DTitreDocumentLu)
						If ($vb_OK)
							//• On teste l'existence dans dossier lu et dossier site le mois (AAAAMM
							vt_DTitreDocumentLu:=vt_DTitreDocumentLu+$va_Temp
