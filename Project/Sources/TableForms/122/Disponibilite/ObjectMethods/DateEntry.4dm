...
Case of 
	...
	: (Form event code:C388=Sur données modifiées:K2:15)
		...
		// On ne peut pas aller plus loin que certaines date
		If (date_d<[HeberGement:5]HG_Date:4)
			date_d:=[HeberGement:5]HG_Date:4
		End if 
		
		If (date_d>[HeberGement:5]HG_RéservationDateFin:134)
			date_d:=[HeberGement:5]HG_RéservationDateFin:134
		End if 
		
		If (Bool:C1537(Form:C1466.appelExterieur)=False:C215)
			POST OUTSIDE CALL:C329(Current process:C322)
		End if 
		
		OB REMOVE:C1226(Form:C1466; "appelExterieur")
End case 