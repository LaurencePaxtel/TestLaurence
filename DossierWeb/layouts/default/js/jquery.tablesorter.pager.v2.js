	...
	$.extend({ tablesorterPager: new function() {
		...
		getAjaxUrl = function(table, c) {
			if ( typeof(c.customAjaxUrl) === "function" ) {
				c.ajaxUrl = c.customAjaxUrl(table, url);
				}
			...
			if (sortCol) {
				...
				$.each(sl, function(i,v){
					arry.push(sortCol +'='+ v[0] + '&direction=' + v[1]);
				});
				// if the arry is empty, just add the col parameter... "&{sortList:col}" becomes "&col"
				url = url.replace(/\{\s*sort(?:List)?\s*:\s*(\w*)\s*\}/g, arry.length ? arry.join('&') : sortCol );
				url = url.replace('direction=0','direction=desc');
				url = url.replace('direction=1','direction=asc');
				arry = [];
			}
			if (filterCol) {
			...
			if ( typeof(c.customAjaxUrl) === "function" ) {
				...
				// url = url.replace(/\{page([\-+]\d+)?\}/, function(s,n){ return c.page + (n ? parseInt(n, 10) : 0); });
				// url = url.replace(/\{size\}/g, c.size);
			}
			return url;
		},
