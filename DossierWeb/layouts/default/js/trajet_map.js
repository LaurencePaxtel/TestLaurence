$(function(){
	...
	function get(choix) {
		...
		var key_vals = url.split('&');
		for(var index in key_vals) {
			var key_val = key_vals[index].split('=');
			var key = key_val[0];
			var val = key_val[1];
			if(choix == key || '?'+choix == key) {
				return val;
			}
		}
		return false;
	...
	var adrorigin = decodeURI(get('adrorigin'));
	var adrdest = decodeURI(get('adrdest'));
	$('#origin').val(adrorigin);
	$('#destination').val(adrdest);
	$('#calculate').click(); 
