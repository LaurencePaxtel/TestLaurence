			...
			if(methode_ob_affect == '' & methode_refresh !== 'refresh_object') {
				...
				if( valeurIni !== valeurCurr || $('#'+methode_ob_depart).hasClass('go') || methode_event =='chargement' || methode_ob_depart == 'valider' && !$('#'+methode_ob_depart).hasClass('wait')) {
					...
					success:function(data){
						...
						// {'type':'data','value':'dvfbhfghgfhgf'}
						// {'type':'redirect','value':'link'}
						/*
						var type  = obj.type;
	...
	setInterval(function(){
		...
		// Gestion du clic droit
			setTimeout(GestionClicDroit,100);
		...
		function GestionClicDroit(){
			...
			if(!jQuery.isEmptyObject(Menu)){
				var prefix	=	$('[data-waction]:visible').eq(0).data('waction'); 
				if(prefix != undefined){
					switch(prefix){
						case 'TACHL':
							...
							getContextMenu(function(key, options) {
								...
								switch(action){
									case 'add':
										$('[data-waction="TACHL"] .btn_add').click();
									break;
									case 'edit':
										$('[data-waction="TACHL"] .btn-modifier-ligne').click();
									break;
									case 'del':											
										$('[data-waction="TACHL"] .btn_sup').click();
									break;
									case 'testrefresh':											
										var uidFiche	=   $('[data-uidm]').data('uidm');
										var urlAjax = 'http://' +document.location.host+'/DOSP?action='+action+'&uidm='+uidFiche;
										/*console.log(action);
										console.log(urlAjax);*/
										$.ajax({
											type: 'GET',
							...
							getContextMenu(function(key, options) {
								...
								switch(action){
									...
									case 'testrefresh':											
										console.log('ok');
										var uidFiche	=   $('[data-uidm]').data('uidm');
										var uidLigne	= 	$tr.find('td:first').text();
										console.log(uidLigne);
										var urlAjax = 'http://' +document.location.host+'/SOC?action='+action+'&uidm='+uidFiche+'&uidligne='+uidLigne;
										/*console.log(action);
										console.log(urlAjax);*/
										$.ajax({
											type: 'GET',
									...
									case 'newpage':
										...
										var uidLigne	= 	$tr.find('td:first').text();
										var urlAjax = 'http://' +document.location.host+'/INT?action=edit&uid='+uidLigne;
										
										$('#blank-link').attr('href',urlAjax);
										dispatchClick($('#blank-link')[0]);
									break;
								}
							},Menu.INTL);
						...
						default:
							$.contextMenu( 'destroy', '.table td' );
							$('.table td').unbind("contextmenu");
					}
				}
			}
		}
	...
	$('.table-ligne').on('refreshFromCol', function(event, response) {

		object = eval(response);
		var valeur = object.valeur;
		Object.keys(valeur).forEach(function (key) {
			$(key).replaceWith(valeur[key]);
			// $(key).after(valeur[key]);
		});
		
		// $(this).off('refreshFromCol');
								
	});
	
	function traitement_objects (tb_obj) {
